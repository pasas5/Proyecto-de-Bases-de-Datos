...
\subsection{Pruebas de Procedimientos Almacenados}
...
\begin{itemize}
    \item \textbf{Intento de reseña sin compra previa:} Al ejecutar el procedimiento \texttt{sp\_registrar\_reseña} con un cliente que no ha adquirido el producto especificado, el sistema bloquea la transacción y arroja correctamente el error \textit{SQLSTATE 45000: Solo los clientes que han comprado el producto pueden dejar una reseña}.
    \item \textbf{Límite de pedidos:} Se probó el procedimiento \texttt{sp\_registrar\_pedido} para un cliente que ya contaba con 5 pedidos en estado "pendiente". El sistema detuvo la inserción, cumpliendo con la regla de negocio establecida.
\end{itemize}

\subsection{Impacto de los Índices (Análisis con EXPLAIN)}
Para optimizar el rendimiento de la base de datos, se crearon tres índices no agrupados. El impacto de estos índices se analizó utilizando el comando \texttt{EXPLAIN} en MySQL:
\begin{enumerate}
    \item \texttt{idx\_productos\_categoria}: Al buscar productos filtrando por \texttt{id\_categoria}, el \texttt{EXPLAIN} demostró que el motor dejó de hacer un \textit{Full Table Scan} (recorrer toda la tabla) y pasó a usar el índice (tipo \textit{ref}), reduciendo significativamente la cantidad de filas examinadas.
    \item \texttt{idx\_pedidos\_cliente}: Agilizó los cruces (\textit{JOINs}) entre Clientes y Pedidos, vital para la consulta analítica del historial de compras.
    \item \texttt{idx\_productos\_nombre}: Permitió búsquedas rápidas en el catálogo utilizando la columna de texto.
\end{enumerate}
